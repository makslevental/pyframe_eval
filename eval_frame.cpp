...
typedef struct THPPyInterpreterFrame {
  PyObject_HEAD _PyInterpreterFrame *frame; // Borrowed reference
...
#define DECLARE_PYOBJ_ATTR(name)                                               \
  static PyObject *THPPyInterpreterFrame_##name(THPPyInterpreterFrame *self,   \
                                                PyObject *_noargs) {           \
...
static THPPyInterpreterFrame *
THPPyInterpreterFrame_previous(THPPyInterpreterFrame *self, PyObject *_noargs) {
...
static PyObject *THPPyInterpreterFrame_f_lasti(THPPyInterpreterFrame *self,
                                               PyObject *_noargs) {
...
THPPyInterpreterFrame *THPPyInterpreterFrame_New(_PyInterpreterFrame *frame) {
  ...
  THPPyInterpreterFrame *self =
      (THPPyInterpreterFrame *)type->tp_alloc(type, 0);
  if (!self)
    return NULL;
  self->frame = frame;
  return self;
...
#else
static PyObject *custom_eval_frame_shim(THP_EVAL_API_FRAME_OBJECT *frame,
                                        int throw_flag) {
...
static inline PyObject *call_callback(PyObject *callable,
...
inline static PyObject *eval_custom_code(PyThreadState *tstate,
  ...
  for (Py_ssize_t i = 0; i < frame->f_code->co_nlocalsplus; i++) {
    ...
    if (name == NULL || idx == NULL ||
        (new_i == (Py_ssize_t)-1 && PyErr_Occurred() != NULL)) {
      Py_DECREF(name_to_idx);
      THP_PyFrame_Clear(shadow);
      free(shadow);
      Py_DECREF(func);
...
static PyObject *_custom_eval_frame(PyThreadState *tstate,
  ...
  // TODO(alband): This is WRONG for python3.11+ we pass in a
  // _PyInterpreterFrame even though we should pass a PyFrameObject.
  ...
  // TODO(alband): This is WRONG for python3.11+ we pass in a
  // _PyInterpreterFrame that gets re-interpreted as a PyObject (which it is
  // NOT!)
  PyObject *result =
...
PyObject *torch_c_dynamo_eval_frame_init(void) {
  ...
  if (PyModule_AddObject(module, "_PyInterpreterFrame",
